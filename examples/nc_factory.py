...
score_model(icp_cv, "IcpClassifier", data, "iris", [class_mean_errors, class_avg_c])
...
nc = NcFactory.create_nc(
    RandomForestClassifier(n_estimators=100), normalizer_model=KNeighborsRegressor()
)
icp = IcpClassifier(nc)
icp_cv = ClassIcpCvHelper(icp)

score_model(
    icp_cv, "IcpClassifier (normalized)", data, "iris", [class_mean_errors, class_avg_c]
)
...
nc = NcFactory.create_nc(
    RandomForestClassifier(n_estimators=100, oob_score=True), oob=True
)
icp_cv = OobCpClassifier(nc)

score_model(
    icp_cv, "IcpClassifier (OOB)", data, "iris", [class_mean_errors, class_avg_c]
)
...
score_model(
    icp_cv, "IcpRegressor", data, "diabetes", [reg_mean_errors, reg_median_size]
)
...
nc = NcFactory.create_nc(
    RandomForestRegressor(n_estimators=100), normalizer_model=KNeighborsRegressor()
)
icp = IcpRegressor(nc)
icp_cv = RegIcpCvHelper(icp)
...
nc = NcFactory.create_nc(
    RandomForestRegressor(n_estimators=100, oob_score=True), oob=True
)
icp_cv = OobCpRegressor(nc)

score_model(
    icp_cv, "IcpRegressor (OOB)", data, "diabetes", [reg_mean_errors, reg_median_size]
)
